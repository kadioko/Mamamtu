# Tanzania Pilot Clinic Outreach List

**Purpose:** Target clinics for MamaMtu beta pilot program (free for 3 months, then transition to paid).

**Last Updated:** 2026-05-28 | **Current Phase:** Week 3 — Follow-up

---

## Outreach Status Summary

| Week | Status | Actions |
| --- | --- | --- |
| Week 1 | ✅ Complete | Initial outreach emails drafted and sent to 9 private clinics |
| Week 2 | ✅ Complete | Expanded to faith-based networks (CSSC, BAKWATA) and 3 NGOs (MDH, Amref, Jhpiego) — total 14 emails sent |
| Week 3 | 🟡 Active | Follow-up on all 14 contacts; schedule demos; in-person visits to cold contacts |
| Week 4 | ⬜ Upcoming | Close first signed pilot agreement |

**Total contacted: 14** (9 private clinics, 2 faith-based networks, 3 NGOs)  
**Live tracker:** `docs/MamaMtu_Outreach_Tracker.csv`  
**Pilot agreement:** `docs/MamaMtu_Pilot_Terms.md`  
**Info sheet (print):** `docs/pdfs/MamaMtu_Pilot_Info_Sheet.html`

---

## Target Criteria

**Ideal Pilot Clinic:**
- 5-50 staff members (manageable size for feedback)
- Serves maternal health patients (ANC, delivery, postnatal)
- Some digital readiness (at least one computer/tablet + internet)
- Located in Dar es Salaam initially (easier for in-person support)
- Decision-maker accessible (owner, administrator, or medical director)
- Open to trying new systems (early adopter mindset)

---

## Tier 1: Private Clinics (Primary Targets)

### Aga Khan Health Services Tanzania
| | |
| --- | --- |
| **Locations** | Dar es Salaam, Mwanza, Arusha |
| **Type** | Private hospital chain with maternity services |
| **Why Target** | Strong digital infrastructure, quality focus, East African network |
| **Decision Maker** | Hospital Administrator / Regional Director |
| **Approach** | Partner as a "quality accreditation" story; emphasize reporting and audit features |
| **Contact Strategy** | LinkedIn research → cold email to administrator → follow-up call |
| **Status** | ✉️ Email sent 2026-05-28 — awaiting response |

### Shree Hindu Mandal Hospital
| | |
| --- | --- |
| **Location** | Dar es Salaam (Upanga) |
| **Type** | Private hospital with maternity ward |
| **Why Target** | Community-trusted, moderate size, management accessible |
| **Decision Maker** | Hospital Secretary or Medical Superintendent |
| **Approach** | Community health angle; emphasize continuity of care for local families |
| **Contact Strategy** | Visit in person, request meeting with administrator |
| **Status** | ✉️ Email sent 2026-05-28 — awaiting response |

### Premier Care Hospital
| | |
| --- | --- |
| **Location** | Dar es Salaam (Masaki/Oyster Bay area) |
| **Type** | Private clinic with maternity services |
| **Why Target** | Middle-class patient base, likely tech-forward |
| **Decision Maker** | Clinic Director or Owner |
| **Approach** | Efficiency and patient experience angle |
| **Contact Strategy** | Website contact form + LinkedIn |
| **Status** | ✉️ Email sent 2026-05-28 — awaiting response |

### Dar IVF
| | |
| --- | --- |
| **Location** | Dar es Salaam |
| **Type** | Private fertility and women's health clinic |
| **Why Target** | Women's health focus, experienced with maternal patient base |
| **Decision Maker** | Clinic Director or Administrator |
| **Approach** | Patient records continuity and ANC tracking efficiency |
| **Contact Strategy** | Email sent via initial outreach round |
| **Status** | ✉️ Email sent 2026-05-28 — awaiting response |

### Hans Mgaya Hospital
| | |
| --- | --- |
| **Location** | Dar es Salaam |
| **Type** | Private hospital |
| **Why Target** | Community-established hospital with maternity services |
| **Decision Maker** | Hospital Administrator or Medical Superintendent |
| **Approach** | Operational efficiency and MoH reporting compliance |
| **Contact Strategy** | Email sent via initial outreach round |
| **Status** | ✉️ Email sent 2026-05-28 — awaiting response |

### NuLife
| | |
| --- | --- |
| **Location** | Dar es Salaam |
| **Type** | Private clinic |
| **Why Target** | Modern brand positioning suggests digital readiness |
| **Decision Maker** | Clinic Director or Owner |
| **Approach** | Patient experience and digital workflow modernization |
| **Contact Strategy** | Email sent via initial outreach round |
| **Status** | ✉️ Email sent 2026-05-28 — awaiting response |

### Komakoma Polyclinic
| | |
| --- | --- |
| **Location** | Dar es Salaam |
| **Type** | Private polyclinic |
| **Why Target** | Multi-service clinic likely handling maternal cases |
| **Decision Maker** | Administrator or Medical Director |
| **Approach** | Reduce admin burden across multiple service lines |
| **Contact Strategy** | Email sent via initial outreach round |
| **Status** | ✉️ Email sent 2026-05-28 — awaiting response |

### Kairuki Hospital
| | |
| --- | --- |
| **Location** | Dar es Salaam |
| **Type** | Private hospital |
| **Why Target** | Established private hospital with maternity ward; recognized name |
| **Decision Maker** | Hospital Administrator |
| **Approach** | Quality of care story; reporting, audit, and compliance features |
| **Contact Strategy** | Email sent via initial outreach round |
| **Status** | ✉️ Email sent 2026-05-28 — awaiting response |

### Columbia Africa IST Clinic
| | |
| --- | --- |
| **Location** | Dar es Salaam |
| **Type** | International-standard private clinic |
| **Why Target** | High digital readiness; international quality standards; tech-forward |
| **Decision Maker** | Clinic Director or Administrator |
| **Approach** | Premium reporting, audit trails, and integration with international standards |
| **Contact Strategy** | Email sent via initial outreach round |
| **Status** | ✉️ Email sent 2026-05-28 — awaiting response |

---

### Various Small Private Clinics (Target 10-15)

**Dar es Salaam Neighborhoods to Target:**
- Kinondoni: Magomeni, Tandale, Manzese
- Ilala: Kariakoo, Buguruni, Tabata
- Temeke: Tandika, Mbagala, Chang'ombe

**Search Strategy:**
1. Google Maps: "clinic" + "maternity" + "Dar es Salaam"
2. Ask local health workers for referrals
3. Check Tanzania Medical Association member directory
4. Visit neighborhoods, note clinic signage, collect phone numbers

**Initial Outreach Script:**
```
"Hello, my name is Godfrey Mariki from MamaMtu. We're a Tanzanian health 
tech company developing a digital system to help clinics track maternity 
patients and reduce missed appointments. We're looking for 3-5 clinics 
to pilot our system for free for 3 months. Would [clinic name] be interested 
in learning more?"
```

---

## Tier 2: Faith-Based Hospitals

### Christian Social Services Commission (CSSC) Facilities
| | |
| --- | --- |
| **Network** | 100+ church-affiliated health facilities across Tanzania |
| **Why Target** | Mission-driven, community-embedded, often underserved areas |
| **Decision Maker** | CSSC National Coordinator + individual hospital administrators |
| **Approach** | Align with mission: "technology serving the vulnerable"; emphasize rural reach; Dr. Wiles can support faith-based engagement |
| **Contact Strategy** | Email CSSC headquarters in Dodoma → request introduction to member hospitals |
| **Status** | ✉️ Email sent 2026-05-28 — awaiting response |

### BAKWATA Health Facilities
| | |
| --- | --- |
| **Network** | Islamic health facilities, particularly strong in Zanzibar and coastal areas |
| **Why Target** | Religious trust, established maternal health services |
| **Decision Maker** | BAKWATA health committee + facility administrators |
| **Approach** | Respectful partnership; emphasize serving all communities; Dr. Wiles can support faith-based engagement |
| **Contact Strategy** | Contact BAKWATA headquarters in Zanzibar |
| **Status** | ✉️ Email sent 2026-05-28 — awaiting response |

### Specific Faith-Based Targets

- **Lutheran Hospital, Arusha**
- **Iringa Lutheran Hospital**
- **KCMC (Kilimanjaro Christian Medical Centre)** - Teaching hospital, research angle
- **Benedictine Sisters facilities** (check CSSC list)

---

## Tier 3: Public/NGO Partnership (Longer Sales Cycle)

### Management and Development for Health (MDH)
| | |
| --- | --- |
| **Focus** | HIV/AIDS and maternal health programs, major PEPFAR implementer |
| **Presence** | Multiple regions in Tanzania |
| **Why Target** | Deep health system integration, data-focused, donor-funded |
| **Decision Maker** | Program Director or M&E Lead |
| **Approach** | Integration with existing programs; emphasize reporting and data quality |
| **Contact Strategy** | LinkedIn research → cold email → reference common health conferences |
| **Status** | ✉️ Email sent 2026-05-28 — awaiting response |

### Amref Health Africa Tanzania
| | |
| --- | --- |
| **Focus** | Community health, maternal health, digital health innovations |
| **Why Target** | Innovation-friendly, health tech experience, rural focus |
| **Decision Maker** | Country Director or Digital Health Lead |
| **Approach** | "Innovation partnership" framing; emphasize rural/offline capabilities |
| **Contact Strategy** | Email country office; reference known innovations |
| **Status** | ✉️ Email sent 2026-05-28 — awaiting response |

### Jhpiego Tanzania
| | |
| --- | --- |
| **Focus** | Maternal and newborn health, family planning |
| **Why Target** | Core mandate alignment; research and evaluation capacity |
| **Decision Maker** | Country Director or MNCH Program Lead |
| **Approach** | Pilot as research collaboration; co-publish results |
| **Contact Strategy** | Email with concept note; emphasize M&E and research potential |
| **Status** | ✉️ Email sent 2026-05-28 — awaiting response |

---

## Outreach Templates

### Email Template 1: Private Clinic (Direct)

**Subject:** Partnership Opportunity - Digital Health System for Maternity Care (Free Pilot)

```
Dear [Clinic Administrator Name],

I hope this message finds you well. My name is Godfrey Mariki, and I am the 
Founder & CEO of MamaMtu, a Tanzanian health technology company.

We have developed MamaMtu, a secure digital platform designed specifically 
to help clinics and hospitals manage maternity patient records, track 
antenatal visits, send appointment reminders, and generate reports for 
ministry compliance.

Unlike complex hospital systems that require full-time IT staff, MamaMtu 
is designed for small-to-medium clinics like [Clinic Name]. It works 
offline when internet is unstable and syncs when connection returns.

We are currently seeking 3-5 clinics in Dar es Salaam to pilot MamaMtu 
for FREE for 3 months. After the pilot, clinics can continue at an 
affordable monthly rate (TZS 100,000-150,000/month).

What we offer pilots:
✓ Free system setup and staff training
✓ Priority WhatsApp support
✓ Input into feature development
✓ Case study publicity (if desired)
✓ First 3 months completely free

What we ask:
✓ Use the system with real patients (with consent)
✓ Provide feedback weekly for first month
✓ Allow us to measure impact (appointments tracked, etc.)
✓ Consider continuing if the system helps

Would you be open to a 15-minute call to discuss whether [Clinic Name] 
might be a good fit for this pilot?

I am available [days/times] or can work around your schedule.

Thank you for your time and for the important work [Clinic Name] does 
for mothers and babies in our community.

Best regards,

Godfrey Mariki
Founder & CEO
MamaMtu (by Necuva Group Limited)
Email: gmariki@necuva.com
Phone: +255743910580
Website: mamamtu.co.tz (coming soon)
```

### Email Template 2: NGO/Development Partner (Mission-Driven)

**Subject:** Partnership Proposal: Digital Maternal Health Platform for Tanzania

```
Dear [Program Director Name],

I am writing on behalf of MamaMtu, a Tanzanian social enterprise developing 
digital health solutions for maternal and newborn care.

Our platform addresses a critical gap in Tanzania's health system: small and 
medium-sized clinics lack affordable, easy-to-use digital tools to track 
pregnancies, manage antenatal care, and ensure continuity of care.

[Organization Name]'s work in [maternal health/digital health/rural care] 
is well respected, and we believe there may be strong alignment between our 
technology and your programs.

Proposed Partnership:

We would welcome the opportunity to pilot MamaMtu alongside [Organization]'s 
existing maternal health programs. Specifically, we could:

1. Deploy MamaMtu at 3-5 of your partner facilities
2. Integrate reporting with your existing M&E systems
3. Co-develop features that address your specific program needs
4. Share anonymized data insights for program improvement
5. Co-author a case study on digital health in [urban/rural] Tanzania

MamaMtu is currently grant-funded for development; we are not seeking 
funding from [Organization]. Rather, we seek:
- Introductions to clinic partners
- Technical guidance on health system integration
- Validation of impact through your M&E capacity

We have received interest from COSTECH and are preparing applications to 
[Grand Challenges Canada/UNICEF/other relevant funder]. A partnership with 
[Organization] would significantly strengthen these applications.

Would you be available for a brief call to explore whether a pilot 
partnership might be mutually beneficial?

Thank you for considering this opportunity. We look forward to your response.

Sincerely,

Godfrey Mariki
Founder & CEO
MamaMtu (by Necuva Group Limited)
Email: gmariki@necuva.com
Phone: +255743910580
```

### WhatsApp Template: Quick Introduction

```
Jambo [Name],

Ninaitwa Godfrey Mariki kutoka MamaMtu - mfumo wa kidigitali kwaajili ya 
kuhusaidia vituo vya afya kufuatilia wajawazito na kukumbusha wateja 
miadi yao.

Tunatafuta kliniki 3-5 za kuanzia kama waendeshaji wa kwanza (pilot) 
katika Dar es Salaam kwa miezi 3 bila malipo. 

Je, [Clinic Name] ingependa kujifunza zaidi?

Godfrey Mariki
MamaMtu (Necuva Group Limited)
+255743910580
```

---

## Action Plan: Next 30 Days

### Week 1: Preparation ✅ COMPLETE
- [x] Finalize pilot terms (what's free, what happens after 3 months)
- [x] Create one-page pilot information sheet (English + Swahili)
- [x] Set up tracking spreadsheet for outreach
- [x] Draft and customize email templates

### Week 2: Outreach Begins ✅ COMPLETE
- [x] Send emails to private clinics — **9 sent** (Aga Khan, Shree Hindu Mandal, Premier Care, Dar IVF, Hans Mgaya Hospital, NuLife, Komakoma Polyclinic, Kairuki Hospital, Columbia Africa IST Clinic)
- [x] Send 2 emails to faith-based networks (CSSC + BAKWATA)
- [x] Send 3 emails to NGOs (MDH, Amref, Jhpiego)
- [x] Make 3 follow-up phone calls
- [x] All emails CC'd gmariki@necuva.com; recipients can reply-all or directly to that address
- **Total sent: 14** (9 private clinics + 2 faith-based + 3 NGOs)

### Week 3: Follow-Up
- [ ] Follow up on non-responses
- [ ] Schedule 2-3 meetings with interested clinics
- [ ] Visit 1-2 clinics in person if no email response
- [ ] Refine pitch based on feedback

### Week 4: Close First Pilots
- [ ] Host first clinic demo meeting
- [ ] Sign pilot agreement with first clinic
- [ ] Begin technical setup for first clinic
- [ ] Continue outreach until 3-5 clinics committed

---

## Tracking Spreadsheet

**Live tracker:** `docs/MamaMtu_Outreach_Tracker.csv` — all 14 contacts with date, category, response status, and notes.

**Current counts (2026-05-28):**

| Category | Count | Clinics / Orgs |
|---|---|---|
| Private clinics | 9 | Aga Khan, Shree Hindu Mandal, Premier Care, Dar IVF, Hans Mgaya, NuLife, Komakoma Polyclinic, Kairuki Hospital, Columbia Africa IST Clinic |
| Faith-based networks | 2 | CSSC, BAKWATA |
| NGOs | 3 | MDH, Amref, Jhpiego |
| **Total** | **14** | |

**Status key:** Awaiting / Responded — Interested / Responded — Not Interested / Meeting Scheduled / Agreement Signed / Live

---

## Success Metrics for Outreach

- **Week 2:** ✅ **14 emails sent** (9 private clinics, 2 faith-based networks, 3 NGOs) + 3 follow-up phone calls — DONE. All emails CC'd gmariki@necuva.com.
- **Week 3:** 3 responses received, 2 meetings scheduled
- **Week 4:** First pilot agreement signed
- **Month 1:** 3-5 pilot clinics committed
- **Month 2:** First clinic live with real patients
- **Month 3:** All 3-5 clinics active, feedback collected

---

## Tips for Tanzania Clinic Outreach

1. **Lead with relationships**
   - Warm introductions beat cold emails
   - Ask your network: "Do you know anyone who works at a clinic?"

2. **Visit in person**
   - Many clinic administrators don't check email regularly
   - A respectful in-person visit can be more effective

3. **Speak Swahili**
   - Even if the administrator speaks English, starting in Swahili builds rapport

4. **Emphasize free pilot**
   - "No cost, no commitment for 3 months" lowers barriers

5. **Show, don't just tell**
   - Bring a tablet, show the demo live
   - Let them touch and use it

6. **Address concerns proactively**
   - "What if internet goes down?" → Show offline capability
   - "What about patient privacy?" → Show compliance docs
   - "What if we don't like it?" → No obligation after 3 months

7. **Follow up persistently**
   - Healthcare administrators are busy
   - Polite follow-up after 3-5 days is normal

---

## Next Steps (Current — Week 3)

1. **Follow up** on the 14 sent emails — 3–5 day cadence; WhatsApp follow-up where email address is uncertain
2. **Schedule 2–3 demo meetings** with any respondents — bring tablet, show live demo
3. **Visit 1–2 clinics in person** for those who have not replied to email
4. **Refine pitch** based on objections and questions heard so far
5. **Goal:** Sign first pilot agreement before end of Week 4
6. **Leverage Dr. Jade Wiles Jr.** for faith-based clinic (CSSC, BAKWATA) engagement strategy and NGO partnership framing

**Remember:** The first 3 clinics are the hardest. After that, referrals and testimonials make the rest easier.
